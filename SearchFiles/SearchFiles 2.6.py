...
#
# Configure the search job by defining constants in the sections below
...
# ========================================================================
# Directory where the search is started
# ========================================================================
...
# ========================================================================
# Filter on other file properties
#
# Use the uppercase versions of the field names
# The default file properties are always present
# PDF_DATE, EXIFDATE, WIDTH and HEIGHT are only present if they are included in EXPORT
# ========================================================================
import re
# FILTER_FILE = lambda f: True
# FILTER_FILE = lambda f: f['MODIFIED'] > '2026-01-01'
# FILTER_FILE = lambda f: f['EXT'] in ['.xlsx', '.jpg', '.jpeg', '.png', '.tiff', '.tif', '.cr2', '.arw']
# FILTER_FILE = lambda f: f['EXT'].lower() in ['.jpg', '.jpeg', '.png'] and f['MODIFIED'] > '2025-01-01'
# FILTER_FILE = lambda f: "collective" in f['FILE'].lower()
# FILTER_FILE = lambda f: "numbering" in f['FILE'].lower() and "word" in f['FILE'].lower()
# FILTER_FILE = lambda f: "litho" in f['FILE'].lower() or "lb" in f['FILE'].lower() or "hybrid" in f['FILE'].lower()
# FILTER_FILE = lambda f: "8800" in f['FILE']

# \b is a word boundary in regex, so "yield" won't match "dry-yield" or "field" unless it's a separate word.
# FILTER_FILE = lambda f: re.compile(r'\byield\b', re.IGNORECASE).search(f['FILE'])
...
FILTER_FILE = lambda f: re.compile(r'\bV\d{1,3}\b', re.IGNORECASE).search(f['FILE'])


# ======================================================================================
# Which file data to export.
# The top line has all available fields.
# Case insensitive, except for the FILTER_FILE function
# ======================================================================================
...
# ========================================================================
# Shows the filename but hyperlinks the full path
# only relevant if HYPERLINK is included in EXPORT
# ========================================================================
...
# V2.4: Included filtering filenames
# V2.5: Included friendly name option in hyperlink
# V2.6: Extended filtering to all fields instead of just filename
#
# To do:
#
# ========================================================================
...
FIELD_FUNCTIONS = {
    ...
    'HYPERLINK': lambda file, date_fmt: f'=HYPERLINK("{file}","{os.path.basename(file)}")' if FRIENDLY_HYPERLINK else f'=HYPERLINK("{file}")',
...
def search_files( dir = False,
    ...
    extended_fields = set(fields_upper + ['FULLPATH', 'PATH', 'FILE', 'EXT', 'HYPERLINK', 'MODIFIED', 'ACCESSED', 'CREATED', 'SIZE'])
    export_files = [{field: get_field(file, field, date_fmt) for field in extended_fields} for file in files]

    # Apply flexible filter on full file metadata
    export_files = [f for f in export_files if FILTER_FILE(f)]
    ...
    if len(export_files)==0:
        ...
        print(f'{len(export_files)} files found. Preparing export...', end=' ')
